...
func TestCSITemplateRenderedNodeDaemonset(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Kubernetes 1.34 Linux Only",
			args: args{
				...
				expectedImages: []string{
					"rancher/mirrored-sig-storage-csi-node-driver-registrar:v2.14.0",
					"rancher/mirrored-cloud-provider-vsphere-csi-release-driver:v3.4.0",
					"rancher/mirrored-sig-storage-livenessprobe:v2.16.0",
				},
			},
		},
		{
			name: "Kubernetes 1.34 Linux and Windows",
			args: args{
				values: map[string]string{
		...
		{
			name: "Kubernetes 1.33 Linux Only",
			args: args{
				values:         map[string]string{"vCenter.clusterId": random.UniqueId()},
				kubeVersion:    "1.33",
				namespace:      "csitest-" + strings.ToLower(random.UniqueId()),
		...
		{
			name: "Kubernetes 1.33 Linux and Windows",
			args: args{
				values: map[string]string{
					"vCenter.clusterId":         random.UniqueId(),
					"csiWindowsSupport:enabled": "true",
	...
	}{
		{
			...
			args: args{
				...
				expectedImages: []string{
					...
					"rancher/mirrored-cloud-provider-vsphere-csi-release-driver:v3.4.0",
					"rancher/mirrored-sig-storage-livenessprobe:v2.16.0",
					"rancher/mirrored-cloud-provider-vsphere-csi-release-syncer:v3.4.0",
					"rancher/mirrored-sig-storage-csi-provisioner:v5.3.0",
				},
			},
		},
		{
